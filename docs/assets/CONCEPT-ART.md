...
| `mist-concept-mark-doorlight-v2.1.jpg` | **proposed (mark, dankefox — parallel candidate, current)** | v2 with the door's left stroke thickened (~1.9×) so it reads as a door frame that could swing, not a wire — per human-reader feedback ("the left line could be sturdier, like a door that opens and closes"). Done in-place on the v2 image: only the solid left/upper-left stroke is dilated, tapering back to the original weight where the line breaks into dots; glow, dots and paper are pixel-identical to v2. |
| `mist-concept-mark-doorlight-v2.jpg` | superseded by v2.1 (mark, dankefox) | v2 of "door with a light inside", larger and bolder: the arch line's right half dissolves into a drift of ink dots carried by a breeze; the amber glow inside spills a little onto the doorstep. Sole color is the glow. Same idea as v1, filled to ~65% of frame. |
| `mist-concept-mark-doorlight-v2-poster-paper.jpg` / `-white.jpg` | derived (poster-size, 2048×3072) | the v2.1 door lifted from its source (alpha-extracted, no redraw), upscaled 2.5× (was 2×; less empty white, the farthest drifting dots still inside the frame), composed on paper / on white. No text, by request; a lettered version can be overlaid as vector later. |
| `mist-concept-mark-doorlight-v2-icon-test.png` | test | 128 / 64 / 32 / 16 px of v2.1 (thicker stroke holds up noticeably better at 32). |
| `mist-concept-mark-doorlight.png` | superseded by v2 (mark, dankefox) | "door with a light inside": one hand-drawn arch line on a ground line, the right half dissolving into ink dots; inside, low, one soft amber glow — the only color. The door dissolves, the light stays. Concept-level; needs a thicker vector stroke to survive 16px (see icon test). |
