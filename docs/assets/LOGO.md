...
正式 SVG 是从手绘原稿描出的轮廓路径（保留作者的手），线比原稿加粗一档、转折与线头为圆形；`#house`（房框）与 `#mist`（字）分组，颜色各自可改；透明底。

**颜色**：`mist` 字目前用的粉蓝（`#7d9bd6`）是暂定值，可以改；建议最终跟其他 visual 与 UI 使用的主题色统一——只改 `#mist` 组的 fill 即可，房框保持墨色、底保持纸色。
