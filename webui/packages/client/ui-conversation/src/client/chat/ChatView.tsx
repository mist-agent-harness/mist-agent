...
//
// Process folding (dsh-folded-chat contract, fold-groups.ts): runs of ≥2
// consecutive process rows (tool calls, bodiless assistant steps) render
// under one FoldGroupRow disclosure instead of as loose rows. Seats stay
// mounted while folded; grouping derives from Node contents, so this view
// also subscribes the Node value list — memoized Seats still absorb the
// per-chunk re-render.
//
...
import type {
  AssistantBlock, ChatConversationViewNode, ConversationTimelineSnapshot,
...
import { FoldGroupRow } from './FoldGroupRow.tsx'
import { groupChatFlow } from './fold-groups.ts'
import type { FoldRowDescriptor } from './fold-groups.ts'
...
/** An assistant step has visible body only through a non-blank text block or
 * an image; reasoning folds natively (Think row) and is not body here. */
function assistantHasVisibleBody(blocks: readonly AssistantBlock[]): boolean {
  return blocks.some(block => block.kind === 'image' || (block.kind === 'text' && block.text.trim() !== ''))
}

/** Fold inputs for one rendered Node (dsh-folded-chat process test inputs). */
function flowRowDescriptor(key: string, node: ChatConversationViewNode | undefined): FoldRowDescriptor {
  if (node?.kind === 'assistant-step') {
    const data: AssistantChatData = (node as ChatNode<'assistant-step'>).data
    return { key, kind: node.kind, hasVisibleBody: assistantHasVisibleBody(data.blocks), running: data.status === 'running' }
  }
  if (node?.kind === 'tool-call') {
    const data: ToolChatData = (node as ChatNode<'tool-call'>).data
    return { key, kind: node.kind, hasVisibleBody: false, running: isRunningTool(data.root) }
  }
  return { key, kind: node?.kind ?? 'unknown', hasVisibleBody: true, running: false }
}
...
export function ChatView({
  ...
  // Node value list: the reactive invalidator for fold grouping. The store
  // itself is a stable live reader; its values array changes identity exactly
  // when some Node's contents do (stream chunk, tool lifecycle).
  const nodeList = useSession(s => s.chat.nodes.values())
  ...
  // dsh-folded-chat grouping over the ordered rows: a cheap O(n) rescan when
  // rows or Node contents change; the memoized Seats absorb the re-render.
  const flowItems = useMemo(() => {
    const byKey = new Map(nodeList.map(node => [node.key, node]))
    return groupChatFlow(order.map(key => flowRowDescriptor(key, byKey.get(key))))
  }, [order, nodeList])
  ...
  // Fold-group manual open/collapse choices keyed by group id (the first
  // member's key). v1: component-local only — not persisted, not in settings.
  // Entries outlive their group harmlessly: a group re-formed on the same
  // leading row legitimately inherits the reader's earlier choice for it.
  ...
  /** One keyed business-row seat, standalone or inside a fold group. */
    ...
    <div className={css.root}>
      <div ref={listRef} className={css.scroll}>
        <div ref={columnRef} className={css.column} data-chat-flow="">
          ...
          {flowItems.map(item => item.type === 'row'
            ? renderSeat(item.key)
            : (
              <FoldGroupRow
                key={item.id}
                ...
                open={foldOverrides.get(item.id) ?? item.running}
                onToggle={() => {
                  setFoldOverrides((previous) => {
                    const next = new Map(previous)
                    next.set(item.id, !(previous.get(item.id) ?? item.running))
                    return next
                  })
                }}
                t={t}
              >
                {item.keys.map(renderSeat)}
              </FoldGroupRow>
            ))}
