...
/**
 * Keep full `file:line` diagnostics on stdout in CI: the GitHub annotation
 * channel caps at 10 entries per step and reached us stripped of paths
 * （后端154 forensics), so stdout is the only complete record. Under
 * GITHUB_ACTIONS with no caller-requested format, pin `--format=unix`
 * (`path:line:col: message [rule]` per line, grep-friendly). Local runs keep
 * the default output.
 * @param args - Oxlint CLI arguments requested by the caller.
 * @param env - Environment inherited by the Oxlint process.
 * @returns args plus the CI format pin when applicable.
 */
function withCiDetailFormat(args: readonly string[], env: NodeJS.ProcessEnv): readonly string[] {
  if (env.GITHUB_ACTIONS !== 'true') return args
  if (args.some(arg => FORMAT_FLAGS.has(arg) || arg.startsWith('--format='))) return args
...
/**
 * Apply the repository worker bound to both Oxlint backends, plus the CI
 * detail-format pin (see withCiDetailFormat).
 * @param args - Oxlint CLI arguments requested by the caller.
 * @param env - Environment inherited by the Oxlint process.
