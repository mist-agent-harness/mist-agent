...
// Vendor-cut orphans: suites whose live fixtures sit in upstream members the
// vendor cut deliberately dropped (see the "Dropped members" header in
// pnpm-workspace.yaml). The spec files stay byte-verbatim in the tree for
// upstream diff fidelity; they are excluded from the inventory here, with
// 后端157 as the record. Restoration means reversing the documented cut
// (re-adding the examples/ workspace member + lockfile), not piecemeal skips.
const vendorCutOrphanTests = [
  // Spawns examples/acp-agent/tests/fixtures/shell/tool-pwsh/{driver.ts,cordis.yml}
  // as a real subprocess; examples/ was dropped at vendor time (47f9438).
