...
- `telegram-channel-acceptance-adversarial.test.ts` 固定五十二种已知假绿；每种作弊都要在无故障正对照通过时被对应灯判红。
...
| TG-02 | 分别制造无绑定、冲突绑定、scope 不可见、撤权和旧代际，并在 advance 前后各完成当前代际请求 | 安全集成 | 有效绑定派发；advance 返回值、新请求代际与成功完成三者一致；五种负例稳定 fail-closed，不猜 resident |
| TG-03 | 冻结 scope/window 初值后让有效 update 经信道进入宿主 | 宿主集成 | 派发身份逐字段等于入站前冻结的 binding/scope/window/source message；插件自建宿主数为零 |
| TG-04 | 重复/乱序 update，并在 advance 前后各完成当前代际请求，再结算旧请求 | 集成 | 三个唯一 update 恰有三份 effect；新旧代际读回一致，当前请求能完成；重复与迟到不新增 effect |
...
| TG-08 | 先做 token 真实解析与可见出站，再建立在途入站/出站；先单独撤 credential，再撤 binding | 并发＋撤权 | credential 状态/代际独立推进；三次边界快照均无 canary；新旧请求稳定拒绝且 inbound/outbound effect 集合不增 |
| TG-09 | 完成一次真实可见出站并读耐久账，再让 Bot API 不可用、尝试出站并再次读账 | 观测集成 | 版本、能力、binding、最近收发可查；成功与失败的返回值、耐久回执和 typed observability 彼此一致 |
...
| GD-02 | 冻结完整 binding 后让一位 resident 更换 model/provider/runtime session，再发言并读取住户普查 | 端到端 | binding 全字段与 residentId/hash 不变；session 变化；普查逐字段等于切换结果，人数不增且无影子 resident |
...
| GD-04 | 冻结 group/address 后，同轮安排一位发言、一位沉默、一位失败 | 群聊集成 | 每条 trace 的 group/address/resident/scope 与冻结计划对应；outbound 分别为 visible/null/rejected |
