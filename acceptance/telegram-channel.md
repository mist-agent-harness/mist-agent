...
- `telegram-channel-acceptance-adversarial.test.ts` 固定三十七种已知假绿；每种作弊都要在无故障正对照通过时被对应灯判红。

## D26：Telegram 信道边界

| 灯 | 合成输入与操作 | 层级 | 可观察通过判据 |
| --- | --- | --- | --- |
| TG-01 | 同一 resident/scope 绑定 chat 与 topic 地址，实际入站一次，再把已见 message/user id 分别塞进 chatId 与 topicId | 绑定集成 | chat/topic 解析回 `(residentId, scopeId)`；已见 message/user id 四种地址形态都稳定拒绝，不能升格成身份或地址 |
| TG-02 | 分别制造无绑定、冲突绑定、scope 不可见、撤权和旧代际，并先完成一条当前代际在途请求 | 安全集成 | 有效当前绑定实际派发；当前代际可完成且 advance 确实推进；五种负例稳定 fail-closed，不猜 resident |
...
| TG-05 | 用四条 host-issued dispatch 分别制造 submitted、accepted、visible 与回执丢失，并逐条耐久读回 | 传输集成 | 返回值与耐久账中都只有 visible 有 Telegram message id；submitted/accepted 不被后台升级；丢回执按冻结的原 binding/target 耐久 unknown |
...
| TG-08 | 先做 token 真实解析与可见出站，再建立在途入站/出站；先单独撤 credential，再撤 binding | 并发＋撤权 | credential 状态/代际独立推进且 active binding 也不能再出站；新入站、新出站和两条旧请求稳定拒绝；计数不增、边界无 canary |
| TG-09 | 完成一次真实可见出站并读取完整状态，再让 Bot API 不可用且真实尝试出站 | 观测集成 | 插件/Bot API 版本、能力、binding version、最近真实收发与时间可查；实际失败回执和状态都 typed unavailable |

## D26：首个群聊演示

| 灯 | 合成输入与操作 | 层级 | 可观察通过判据 |
| --- | --- | --- | --- |
| GD-01 | 两位 resident 各走独立模型通道，在同一 group/address 完成一轮并读取宿主派发账 | 端到端 | 每条可见消息按冻结的 group/address 精确对应 resident/scope/model/provider，dispatchId 能在宿主账读回；模型通道彼此独立 |
| GD-02 | 一位 resident 更换 model/provider/runtime session 后再发言，并读取住户普查 | 端到端 | binding 与 residentId/hash 不变；runtime session 确实变化；普查中的同一住户逐字段等于切换结果，人数不增且没有影子 resident |
...
| GD-05 | 冷启动、重连、重复 update 与 Telegram 不可用后恢复 | 恢复端到端 | binding 全字段与 canonical state 不漂移；重复入站不新增 effect；unknown 在冻结原 target 留下唯一耐久 effect，恢复后不自动重放 |
