...
| OI-07 | 同一次投影做 retain/reduce/drop/hold 四种决定，再让 scope turn 实际读取 retain 项 | 集成 | receipt 带 policy version、opaque source handle 与逐项决定；判卷在投影前冻结 persona/memory，正对照证明投影被消费且真源不反写 |
| OI-08 | 住户追加 persona 修订，人类尝试代改；再读历史 | 集成 | 判卷在修订前冻结旧版 id 与正文字节；旧版逐字保留并指向新版，住户修订生效，人类代改拒绝且不污染链 |
...
| 灯 | 合成输入与操作 | 层级 | 可观察通过判据 |
| --- | --- | --- | --- |
| MC-01 | 住户票和两方关系票齐全时，让六类 machine conformance 中一类失败，再跑全部通过正对照 | 集成 | 账面恰有六个唯一 key；失败时 migration 与 candidate 都不激活；成功后 candidate 指回仍 active 的 source resident，三类判词完整耐久 |
...
| MC-10 | 私密 source 经 opaque handle 投影，检查完整 Result、评测存储和公开输出 | 集成 | 正对照能使用获准内容；完整返回对象、持久记录、日志和公开输出都不复制原文 |
| MC-11 | 先证明原权威 source 可读并完成一次私密评测，再撤权、读取旧 receipt、尝试重新展开原文 | 集成＋恢复 | receipt 精确对应本次 rubric、target、住户/关系判词与计数；完整投影及撤权失败 Result 无原文，撤权后不可展开或重跑 |
| MC-12 | 多人 source 仅获部分 owner grant，再补齐；随后改变 model/provider 版本并重跑三类判词 | 集成 | 所有完整投影 Result 与存储无原文；失败时 candidate 不激活；成功时 candidate 指回 source resident，现行三类判词和旧历史仍完整 |
